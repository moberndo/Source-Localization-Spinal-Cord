...
def LoadData(data_path, type): 
    ...
    def LoadParticipantData(participant_folder_path):
        ...
        return data_marker, data_EEG, data_SCP, 1
    ...
    if type == 'single':
        data_marker, data_EEG, data_SCP, num_participants = LoadParticipantData(data_path)
        ...
        participant_folder_path = glob(join(data_path, "*", ""))
        num_participants = len(participant_folder_path)
        print(f'{num_participants} participants found.')

        for par_folder_path in participant_folder_path:
            data_marker, data_EEG, data_SCP, _ = LoadParticipantData(par_folder_path)
            data_participants_marker.append(data_marker)
            data_participants_EEG.append(data_EEG)
            data_participants_SCP.append(data_SCP)
